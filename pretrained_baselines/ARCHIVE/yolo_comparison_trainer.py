...
class BarcodeDetectionExperiment:
    """Single YOLO model training experiment with wandb logging"""

    def __init__(self, config: TrainingConfig, project_name: str = "barcode-detection"):
        self.config = config
    ...
    def _generate_tags(self) -> list:
        ...
        # Model size tag
        if 'n' in self.config.model_name:
            tags.append('nano')
        elif 's' in self.config.model_name:
            tags.append('small')
        elif 'm' in self.config.model_name:
            tags.append('medium')
    ...
    def train(self):
        ...
        train_args = {
            ...
            # Logging
            'project': 'runs/yolo_base', #local save path
            'name': self.config.experiment_name,
            'exist_ok': True,
            'verbose': True,
            'save': True,
...
class YOLOComparisonTrainer:
    """
    Class to train and compare multiple YOLO model versions (v8 to v12)
    for bounding box detection tasks.
    """

    def __init__(self,
                 base_config: TrainingConfig,
                 yolo_versions: List[str] = None,
                 model_sizes: List[str] = None,
                 project_name: str = "yolo-version-comparison"):
        """
        Initialize the comparison trainer.

        Args:
            base_config: Base training configuration
            yolo_versions: List of YOLO versions to compare (e.g., ['v8', 'v9', 'v10', 'v11', 'v12'])
            model_sizes: List of model sizes to test (e.g., ['n', 's', 'm', 'l', 'x'])
            project_name: Wandb project name
        """
        self.base_config = base_config
        self.project_name = project_name

        self.yolo_versions = yolo_versions

        # Default model sizes
        if model_sizes is None:
            self.model_sizes = ['n', 's', 'm', 'l', 'x']
        else:
    ...
    def _generate_model_configs(self) -> List[Tuple[str, str]]:
        """Generate all combinations of YOLO versions and sizes"""
        configs = []
        for version in self.yolo_versions:
            for size in self.model_sizes:
                # For YOLO v11+ the naming convention changed (no 'v' prefix)
                ...
                configs.append((version, size, model_name))
        return configs

    def _is_model_available(self, model_name: str) -> bool:
        """Check if a YOLO model is available for download"""
        try:
    ...
    def initialize_comparison_run(self):
        ...
        self.comparison_run = wandb.init(
            project=self.project_name,
            name=f"yolo_comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            config={
                "yolo_versions": self.yolo_versions,
                "model_sizes": self.model_sizes,
                "base_config": self.base_config.to_dict(),
                "total_models": len(self.model_configs)
            },
            notes="Comparing YOLO v8-v12 model performance on barcode detection"
        )
    ...
    def train_single_model(self, version: str, size: str, model_name: str) -> Optional[ModelComparisonResult]:
        ...
        config = TrainingConfig(
            ...
            experiment_name=f"{version}_{size}_{datetime.now().strftime('%H%M%S')}",
            **base_dict
        )
        if aug_dict:
    ...
    def run_comparison(self) -> List[ModelComparisonResult]:
        ...
        print(f"🔬 Starting YOLO version comparison")
        ...
        for version, size, model_name in self.model_configs:
            ...
            if result:
                ...
                wandb.log({
                    f"{version}_{size}/training_time": result.training_time,
                    f"{version}_{size}/test_mAP50": result.test_mAP50,
                    f"{version}_{size}/test_mAP50_95": result.test_mAP50_95,
                    f"{version}_{size}/test_precision": result.test_precision,
                    f"{version}_{size}/test_recall": result.test_recall,
                })
    ...
    def _generate_comparison_summary(self):
        ...
        df = pl.DataFrame([{
            'model': r.model_name,
            'version': r.model_name.replace('yolo', '').replace('.pt', '').replace(r.model_size, ''),  # Extract version
            'size': r.model_size,
    ...
    def _create_performance_comparison_plot(self, df):
        ...
        # Use polars pivot operations
        pivot_map50 = df.pivot(values='test_mAP50', index='version', columns='size')
        pivot_map50_95 = df.pivot(values='test_mAP50_95', index='version', columns='size')

        # Convert to pandas for seaborn heatmap
        pivot_map50_pd = pivot_map50.to_pandas()
        pivot_map50_95_pd = pivot_map50_95.to_pandas()

        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))

        # mAP@0.5 heatmap
        sns.heatmap(pivot_map50_pd, annot=True, fmt='.3f', cmap='YlOrRd', ax=ax1)
        ax1.set_title('Test mAP@0.5 by YOLO Version and Size')
        ax1.set_xlabel('Model Size')
        ax1.set_ylabel('YOLO Version')

        # mAP@0.5:0.95 heatmap
        sns.heatmap(pivot_map50_95_pd, annot=True, fmt='.3f', cmap='YlOrRd', ax=ax2)
        ax2.set_title('Test mAP@0.5:0.95 by YOLO Version and Size')
        ax2.set_xlabel('Model Size')
        ax2.set_ylabel('YOLO Version')
    ...
    def _create_training_time_comparison_plot(self, df):
        ...
        df_pd = df.to_pandas()
        ax = sns.barplot(data=df_pd, x='model', y='training_time', hue='version')
    ...
    def save_comparison_results(self, output_path: str = "yolo_comparison_results.json"):
...
# Example usage:
"""
Example usage of YOLOComparisonTrainer:
...
trainer = YOLOComparisonTrainer(
    ...
    yolo_versions=['v8', 'v9', 'v10', 'v11'],  # Skip v12 if not available
